...
- コンパイル済みライブラリ パッケージでの `Invoke Workflow File` サポートを改善しました。
- 相対ワークフロー パスを、プロジェクトのサブフォルダーに格納されているコンパイル済みワークフロー アクティビティに直接解決できるようにしました。
- コンパイル済みの子ワークフローが、実行時に外部の XAML ファイルを見つけて読み込むことに依存しないようにしました。
