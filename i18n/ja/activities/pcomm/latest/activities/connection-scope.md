...
**注意:** このアクティビティは 2 つの接続モードをサポートしています:
* **新しいセッションの開始:** **Profile Path** にワークステーション プロファイル（`.ws` ファイル）のパスを指定します。**Connection Name** を空のままにした場合、新しく起動されたセッションが自動的に検出されます。
* **既存のセッションへのアタッチ:** **Profile Path** を空のままにし、**Connection Name** に文字識別子（A-Z）を指定して、既に実行中のセッションにアタッチします（**Connection Name** も空の場合は、最も直近に開かれたセッションにアタッチします）。
