...
- 修正: Invoke Workflow File アクティビティを使用してコンパイルしたライブラリに関する問題を修正しました。
- 修正: デバッガーで必須引数を使用した際の起動時ログに関する問題を修正しました。
- 追加: 新規プロジェクト作成時の既定のパッケージとして Core 3.6.0 を追加しました。
...
- 追加: Windows 10/11 で、Studio と Agent のユーザーごとのライセンス ファイルに対応しました。
- 更新: ライセンス ファイルに対するすべての操作で UTF-8 を使用するように変更しました。
- 更新: 破損のリスクを軽減するため、ライセンスと設定の書き込みを物理ストレージに確実に反映するようにしました。
- 修正: 設定内のキーが無効またはプレースホルダーの場合、`.lic` ファイル内の有効なキーを使用するようにしました。
- 修正: ライセンス認証に成功した場合のみ、ライセンス設定を保存するようにしました。
- 修正: 設定の破損を検出した後もライセンス ウィザードを操作できるようにし、新しい情報を入力してファイルを置き換えられるようにしました。

## v3.2.2.0

**Studio**

- ワークフロー ランナー、デバッガー、ビジュアル トラッキング、ブレークポイント処理で新しい Return アクティビティに対応しました。これにより、ワークフロー ファイルを途中で終了しても、出力引数を取得できるようになりました。
- パッケージ マネージャーに誤ったパッケージ バージョンの詳細が表示される問題を修正しました。詳細パネルがバージョン ドロップダウンに追従するようになり、バージョン比較を正規化しました。また、公開メタデータがないフィード（ローカル フォルダー、未掲載バージョン）では、ディスク上のコピーからリリース日を取得するようになりました。
- ジェネリック型に対する C# 式エディターの補完を修正しました。変数スコープが実際の CLR 型で型付けされ、ジェネリック型名が正しく表示されるようになり、識別子もエスケープされます。
- ワークフローの実行開始時にエラーが発生しても何も表示されずに失敗する問題を修正しました。エラーのメッセージとスタック トレースが出力ペインおよびログ ファイルに書き込まれ、エラーを無視せずジョブが失敗として完了するようになりました。

**Agent**

- 入力引数の設定、Task History、ジョブの Detail タブを含む、再設計された UI に Agent を更新しました。
- 引数の設定ビューに引数の型を追加しました。
- 文字列値を整数引数に割り当てた場合の検証と、設定タブの文字列値に対する 4000 文字制限を追加しました。
- 引数の設定時に `IsRequired` とデフォルト値を確認する処理を追加しました。

**Executor**

- 専用の `ERROR_MISSING_REQUIRED_ARGUMENT` 戻り値コードを追加しました。必須入力引数に指定値もデフォルト値もない場合、ワークフローを開始せず、Agent から起動された実行がすぐに失敗するようになりました。
- コンソール エグゼキューターで Return アクティビティに対応しました。

**Installer**

- インストーラーに同梱されるパッケージとして、`RCA.Activities.Core.3.5.0`、`RCA.Activities.Excel.3.3.0`、`RCA.Activities.Common.4.8.0` を追加しました。

## v3.2.1.0

- 追加: インストーラーにパッケージ `Core 3.4.1` を追加しました。
- 追加: ブラウザ拡張機能 v3.0.5 の CRX ファイルをブラウザ（Chrome、Edge）向けに追加しました。
...
- **コード IntelliSense ワークスペース** を追加し、未知のデータ型に対して **`using` 名前空間の自動検出 / 挿入** を行えるようにしました。
...
- **スラッシュ コマンド**（`/workflow`、`/log`）および **`@` コンテキスト コマンド** により、特定の XAML ファイルとそのスコープに関する情報を取得できるようにし、ヘルプ テキストとプロンプト ガイダンスを追加しました。
- AkaNinja と生成式に対して C# 言語サポートを追加しました。

**プロジェクト形式と移行**
- 従来の `project.json` に加え、**`project.v1.json`** をサポートし、どちらのファイルでもプロジェクトを開けるようにしました。
- 従来の `project.json` をアップグレードする際に、**移行前に確認するダイアログ** を追加し、デフォルトで非表示にできるオプション制御と、テーマ対応・リソース対応・固定サイズのダイアログを実装しました。
- ライセンス不明の問題を解決するため、`Dirkster.MRULib` を 1.30 にアップグレードしました。
...
- **Bot Executor**: `project.v1.json` を優先し、必要に応じて従来の `project.json` にフォールバックするようにしました。
- ターゲット フレームワークを **net472** に統一しました。
...
- WMIC が無効化 / 非推奨になっている場合に **PowerShell** でマシン UUID を取得するようにし、WMIC が無効な形式を返した場合の `IsWmicAvailable` の不具合を修正しました。
- C: ドライブ以外の Windows インストール環境で、`C:\WINDOWS` をハードコードしていた箇所を `%SystemRoot%` に置き換えました。
- Named-pipe の接続プール管理を改善し、安定性を向上させました。
