...
Akabot Agent 2.2.0.7 is the runtime component that lives on a machine — desktop or server — and executes automation workflows dispatched by Akabot Center or triggered locally. Agents can run attended (alongside a human user) or unattended (fully automated, 24/7).
