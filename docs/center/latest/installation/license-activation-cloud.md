...
> 1. **License Activation** (section 3.1 below) — unlocks the akaBot Agent software using your **License Key**.
> 2. **Register & Connect** (section 3.2 below) — links this specific Agent installation to an Agent record on akaBot Center using an **Agent Key**, so it can receive and execute automation tasks.
>
> Completing only Step 1 will activate the software but the Agent will **not** appear as connected/available on akaBot Center. Both steps must be completed.
...
| **4** | **Machine Username** | The OS-level login username of the workstation running the Agent. | String | 50 chars | No | Mandatory when running automated processes (*Unattended Bot*). Enter the Windows account username directly — this is **not** available in the akaBot Agent client and must be obtained from the workstation itself. |
...
> [!TIP]
> **Locating your Machine Name easily:**
> You can find and copy this parameter directly from your **akaBot Agent** client UI:
> 1. Open the **akaBot Agent** desktop client, click the **Settings** (gear icon) in the upper-right corner.
> 2. Select the **Center** tab. Under the **Central Configuration** pane, your machine's preloaded **Machine name** is displayed. Simply copy this value to paste it into the akaBot Center creation form.
>
> **Machine Username** is **not** shown anywhere in the akaBot Agent client — it is entered directly on akaBot Center as the OS-level login username of the workstation, and is only required when the Agent will run **Unattended Bot** processes. It does not need to match anything on the Agent side and is not part of the Agent-to-Center connection handshake.
...
* **Step 2:** Select the **Center** tab and fill out the **Central Configuration** form:
  ...
  - **Agent key:** Paste the **Agent Key** copied from your akaBot Center detail screen (see **Viewing Agent Details and Copying the Agent Key** above).
...
#### **3.2.3. Troubleshooting: Agent Fails to Connect or Connects Incorrectly**

If the Agent stays **Disconnected** after clicking **Connect**, or connects but does not correspond to the machine you expect, this is almost always caused by one of the following — **especially when your organization manages multiple Agents/machines**:

* **Wrong Agent Key copied from a different Agent record.**
  Each **Key** on akaBot Center belongs to exactly **one** Agent record (one Machine Name). If you manage several Agent machines, it is easy to accidentally open the wrong Agent's detail screen and copy **its** Key by mistake. Pasting that Key into a different physical machine will not connect it correctly — the Key is already tied to another machine's Agent record.
  * **Fix:** On akaBot Center, open the **Agents** list, find the Agent record whose **Machine Name** column exactly matches the target computer, click its **View** (eye) icon, and re-copy the **Key** from *that specific record* — do not reuse a Key copied earlier for a different machine.

* **Machine Name mismatch between the Center record and the physical machine.**
  The **Machine Name** entered when creating the Agent record on Center must be the **exact** hostname of the machine that will connect (as shown in the Agent client's **Center** settings tab). Any difference — a leftover value from renaming/reimaging the machine, a typo, or copying the hostname of a different workstation — will prevent the Agent from being correctly recognized.
  * **Fix:** Compare the **Machine Name** field on the Agent's Center settings tab with the **Machine Name** shown on the Agent's detail page on Center. If they don't match, either correct the Machine Name on the Center record, or create a new Agent record using the machine's actual, current hostname.

> [!TIP]
> When troubleshooting a connection issue, always verify the **Key**, **Machine Name**, and **Center URL** on the Agent client against the exact same fields on **that one** Agent's detail page on Center — not against a different Agent record that happens to look similar.
