...
1. **Add the Open Browser activity**: Drag and drop the **Open Browser** activity into the designer panel.
2. **Specify the URL**: In the **Url** input field (or the text box in the body of the activity), enter the target web page URL enclosed in quotes, for example: `"https://akabot.com/"`.
3. **Configure the Browser Type**: Select the preferred browser (e.g., `Chrome`, `Edge`, or `Firefox`) from the **Browser Type** dropdown list in the **Properties** panel.
4. **Build your automation flow**: Inside the **Do** container block, drag other Browser activities (such as [Click](/docs/activities/browser/latest/activities/click.md) or [Type Into](/docs/activities/browser/latest/activities/type-into.md)) to interact with elements on the loaded page.
5. **Output browser session (optional)**: In the **Output Browser** property, create a `Browser` variable (e.g., `myBrowser`) to store the session context if you plan to attach to this session later using [Browser Scope](/docs/activities/browser/latest/activities/browser-scope.md).
