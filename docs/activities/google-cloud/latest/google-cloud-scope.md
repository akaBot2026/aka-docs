...
* **Service Account Credentials Mode** - Selects the authentication mode (`AutoDetect`, `ServiceAccountKey`, or `ServiceAccountKeyFromFile`).
* **Do** - The container block where you place the Google Cloud activities that will share this connection context.
...
* **Credentials Mode: ScopeType** - The method used to authenticate with Google Cloud. Select one of the following values:
  ...
  | `ServiceAccountKey` *(default)* | Authenticates using the raw JSON content of a Service Account key, provided directly as a `SecureString` in the **Service Account Key** field. |
...
* **Service Account Key: `InArgument<SecureString>`\*** - The JSON content of the Service Account key. Required when **Credentials Mode** is set to `ServiceAccountKey`.
...
* **Display Name (`String`)** - The display name of this activity in the workflow designer. You can rename it to make your workflow easier to read. Default: `Google Cloud Scope`.

---

## See Also

* [Setting Up Google Cloud Credentials](google-cloud-credentials-setup.md) - Step-by-step guide to generating and configuring Google Cloud credentials.