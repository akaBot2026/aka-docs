---
id: troubleshoot-ask-ai
title: Troubleshoot Ask AI
sidebar_label: Troubleshoot Ask AI
sidebar_position: 1
description: Learn how to troubleshoot the network connection for Ask AI service in akaBot Studio.
displayed_sidebar: studioSidebar
---

# Troubleshoot Ask AI

This document guides users and IT Administrators on how to inspect and troubleshoot network connection issues when using the **Ask AI** service (an intelligent assistant feature that supports code generation, expression explanation, and direct chat) on **akaBot Studio** in enterprise environments.
...
2. Search for the configuration key: **`AskAIWorkerApiUrl`** and retrieve its value (e.g., `https://akabot-chatbot-worker.dev`).
...
```powershell
# Example AI server URL: https://akabot-chatbot-worker.dev

# Verify domain name resolution and connection to Port 443 (HTTPS)
Test-NetConnection -ComputerName akabot-chatbot-worker.dev -Port 443
```
...
| :--------------- | :-------------------------------------------------------------------------------------------------------- | :---- | :------- | :---------------------------------------------------------- |
| **AI Server**    | The address configured under `AskAIWorkerApiUrl` (e.g., `akabot-chatbot-worker.dev`) | `443` | HTTPS    | Sends analysis requests and receives responses from the AI. |
